...
const netcraftBtn = document.getElementById("netcraftBtn");
const urlInput = document.getElementById("url-input");

netcraftBtn.addEventListener("click", function() {
  const url = urlInput.value.trim();
  if (!url) return;
  const encodedUrl = url;
  window.location.href = `https://sitereport.netcraft.com/?url=${encodedUrl}`;
});
...
const getCountryLabel = (code) => {
  ...
  const name = new Intl.DisplayNames(["en-GB"], {
    type: "region"
  }).of(code);
...
scanForm.addEventListener("submit", async function (e) {
  ...
  if (e.submitter.id !== "checkBtn") return;
